...
class EbayAuctionSearcher:
    ...
    EUROPEAN_COUNTRIES = [
        ...
        "IE", "IT", "LV", "LI", "LT", "LU", "MT", "MC", "ME", "NL", "MK", "NO", "PL", "PT", "RO", "SM", "RS", "SK", 
    ...
    def search_ebay_auctions(self, keywords, countries=None, max_price=None, min_price=None, 
                             max_time_remaining=None, categories=None, category_ids=None, condition_ids=None):
        countries = countries or self.EUROPEAN_COUNTRIES
        results = []
        
        for country in countries:
            headers = self._build_headers()
            payload = self._build_payload(keywords, country, max_price, min_price, categories, category_ids, condition_ids)
    ...
    def _build_payload(self, keywords, country, max_price, min_price, categories, category_ids, condition_ids):
        item_filters = [
    ...
    def _filter_items_by_time(self, items, max_time_remaining):
        ...
        for item in items:
            ...
            if max_time_remaining and time_remaining > timedelta(seconds=max_time_remaining):
                continue
    ...
    def _parse_end_time(self, end_time: str) -> datetime:
        ...
        try:
            return datetime.strptime(end_time, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=timezone.utc)
        except ValueError:
            return datetime.strptime(end_time, "%Y-%m-%dT%H:%M:%S.000Z").replace(tzinfo=timezone.utc)
    ...
    def _format_item(self, item, time_remaining):
        return {
            ...
            "category_id": item.get("primaryCategory", [{}])[0].get("categoryId", "Unknown"),
            ...
            "seller_user_id": item.get("sellerInfo", [{}])[0].get("sellerUserName", "Unknown"),
            "feedback_score": item.get("sellerInfo", [{}])[0].get("feedbackScore", "Unknown"),
            "feedback_percentage": item.get("sellerInfo", [{}])[0].get("positiveFeedbackPercent", "Unknown"),
